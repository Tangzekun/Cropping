...
#		return img
	...
	def cropEdges(self,localImage):
		...
		#img = self.zoomOut("/Users/tangzekun/Desktop/Cropped/"+imageName)
		img = Image.open(localImage)
		rgb_im = img.convert('RGB')
		imageScale = img.size
	...
	def detectRelation(self, imageName,croppedImage):
		
		model = Image.open("/Users/tangzekun/Desktop/Cropped/"+imageName)
		...
		croppedImage = ImageDraw.Draw(model)
					
		#completely = model.crop((leftX,topY,rightX,bottomY))
		if (bottomY-topY) > int(scale[1]*0.90):
#			topY += 80
#			bottomY -= 50
			croppedImage.line((leftX,topY,rightX,topY),fill=128,width = 1)
			croppedImage.line((leftX,topY,leftX, bottomY),fill=128,width = 1)
			croppedImage.line((rightX,topY,rightX,bottomY),fill=128,width = 1)
			croppedImage.line((leftX,bottomY,rightX,bottomY),fill=128,width = 1)
		else:
			croppedImage.line((leftX,topY,rightX,topY),fill=128,width = 1)
			croppedImage.line((leftX,topY,leftX, bottomY),fill=128,width = 1)
			croppedImage.line((rightX,topY,rightX,bottomY),fill=128,width = 1)
	...
	def uploadToDB(self,localImage):
		
		encoded_string= open(localImage, 'rb')
...
#		test.uploadToDB(edgedImage)
	...
	if data["images"][clothingInfo][2] == 3:		
		test = Solution() 
		localImage = test.readUrlToImage(data["images"][clothingInfo][0])
		croppedImage = test.cropEdges(localImage)
		deepCroppedImage = test.detectRelation(localImage,croppedImage)
		test.uploadToDB(deepCroppedImage)
db.close()
